...
@tf.function
def train_step(images, fixed_noise):
...
# Generate fixed noise for training
fixed_noise = tf.random.normal([batch_size, latent_dim])

# Train the GAN
for epoch in range(epochs):
    for image_batch in dataset:
        train_step(image_batch, fixed_noise)

    # Save generated images every 100 epochs
    if (epoch + 1) % 100 == 0:
        generated_images = generator(fixed_noise, training=False)
        for i, image in enumerate(generated_images):
            generated_image = tf.cast((image * 255), tf.uint8)
            Image.fromarray(generated_image.numpy()).save(f'GAN 4 generated_image_epoch_{epoch + 1}_batch_{i + 1}.png')
